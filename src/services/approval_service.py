...
from src.models.enums import TestResultStatus, LotStatus, UserRole, AuditAction
...
class ApprovalService(BaseService[TestResult]):
    ...
    def approve_test_result(
        ...
        if test_result.status != TestResultStatus.REVIEWED:
            raise ValueError("Test result must be reviewed before approval")
    ...
    def get_pending_approvals(
        ...
        query = (
            ...
            .join(Lot)
            .filter(TestResult.status == TestResultStatus.REVIEWED)
        )
    ...
    def _check_lot_approval_status(
        ...
        if all_approved:
            # Check if all tests are passing specifications
            all_passing = self._check_all_tests_passing(test_results)
            
            # Update lot status based on test results
            lot = db.query(Lot).filter(Lot.id == lot_id).first()
            if lot and lot.status == LotStatus.PENDING:
                old_status = lot.status
                
                # If all tests pass, go directly to APPROVED
                # If any test fails, go to TESTED (needs review)
                lot.status = LotStatus.APPROVED if all_passing else LotStatus.TESTED
                ...
                AuditLog.log_change(
                    ...
                    user=user_id,
                    reason="Automatic update - all test results approved",
                )

                logger.info(
                    f"Lot {lot.lot_number} automatically updated to {lot.status.value} status"
                )
