...
class LotService(BaseService[Lot]):
    ...
    def create_lot(
        ...
        if validated_data["lot_type"] == LotType.MULTI_SKU_COMPOSITE:
            if not product_ids or not product_percentages:
                raise ValueError(
                    "Multi-SKU composite lots require products with percentages"
                )
            ...
            if abs(total_percentage - 100.0) > 0.01:
                raise ValueError(
                    f"Product percentages must sum to 100, got {total_percentage}"
                )
    ...
    def update_lot_status(
        ...
        valid_transitions = {
            LotStatus.PENDING: [LotStatus.TESTED],
            LotStatus.TESTED: [LotStatus.APPROVED, LotStatus.PENDING],
            LotStatus.APPROVED: [LotStatus.RELEASED, LotStatus.TESTED],
        ...
        if new_status == LotStatus.APPROVED:
            ...
            unapproved_tests = [
                tr for tr in lot.test_results if tr.status != "approved"
            ]
            if unapproved_tests:
                raise ValueError(
