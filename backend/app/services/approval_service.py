...
class ApprovalService(BaseService[TestResult]):
    ...
    def _check_lot_approval_status(
        ...
        if not lot or lot.status not in [
        ...
        if not completeness["is_complete"]:
            ...
            if all_passing:
                reason = (
                    "All required tests present and passing - awaiting manual QC review"
                )
            else:
    ...
    def _check_test_passes_spec(self, result: TestResult) -> bool:
        ...
        if verdict.kind == VerdictKind.PASS:
            return True
        if verdict.kind == VerdictKind.NO_SPEC:
            # Current approval workflows allow ad-hoc/no-spec rows; future release
            # gates should inspect VerdictKind directly and require PASS.
            return True

        logger.warning(
            f"Test '{result.test_type}' did not produce a passing spec verdict "
    ...
    def check_test_completeness(self, db: Session, lot_id: int) -> Dict[str, Any]:
        """
        ...
        completed_tests = [tr.test_type for tr in lot.test_results]
        ...
        missing_required = [spec.lab_test_type.test_name for spec in missing_specs]
    ...
    def update_lot_status_after_results(self, db: Session, lot_id: int) -> None:
        """
    ...
    def validate_lot_for_approval(self, db: Session, lot_id: int) -> Dict[str, Any]:
        """
        ...
        lot = db.query(Lot).filter(Lot.id == lot_id).first()
        if not lot:
            return {"can_approve": False, "issues": ["Lot not found"], "warnings": []}
        ...
        if unapproved_tests:
            issues.append(f"Unapproved test results: {', '.join(unapproved_tests)}")
        ...
        if lot.lot_products:
            ...
            for test_result in lot.test_results:
                validation = product_service.validate_test_result(
                    db, product.id, test_result.test_type, test_result.result_value
                )
        ...
        return {"can_approve": len(issues) == 0, "issues": issues, "warnings": warnings}
    ...
    def create_coa_releases_for_lot(
        ...
        existing_releases = (
            db.query(COARelease).filter(COARelease.lot_id == lot_id).all()
        )
        ...
        lot_products = db.query(LotProduct).filter(LotProduct.lot_id == lot_id).all()
