        ...
        if submitted:
            if not lot_number:
                ...
                try:
                    ...
                    # Get the product to access expiry duration
                    product = db.query(Product).filter(Product.id == product_id).first()
                    
                    # Calculate expiry date based on product's expiry duration
                    from dateutil.relativedelta import relativedelta
                    expiry_months = product.expiry_duration_months if product and product.expiry_duration_months else 36
                    exp_date = mfg_date + relativedelta(months=expiry_months)
...
def master_lot_form(db: Session, lot_service: LotService, products):
    ...
    # Product selection - single dropdown for all sub-batches
    product_options = {f"{p.display_name} (ID: {p.id})": p.id for p in products}
    selected_product = st.selectbox(
        "Product *",
        options=list(product_options.keys()),
        help="Select the product for this master lot - all sub-batches will use this product"
    )
    ...
    if st.button("Submit Master Lot", type="primary"):
        ...
        elif edited_df['Batch #'].eq('').any():
            st.error("Please fill in all required fields")
        else:
            try:
                # Get product to determine expiry duration
                product_id = product_options[selected_product]
                product = db.query(Product).filter(Product.id == product_id).first()
                ...
                from dateutil.relativedelta import relativedelta
                expiry_months = product.expiry_duration_months if product and product.expiry_duration_months else 36
                min_mfg_date = edited_df['Mfg Date'].min()
                exp_date = min_mfg_date + relativedelta(months=expiry_months)
                
                # Prepare parent lot data
                parent_lot_data = {
                    "lot_number": master_lot_number,
                    "lot_type": LotType.PARENT_LOT,
                    "mfg_date": min_mfg_date,
                    "exp_date": exp_date,
                }
                ...
                # Create parent lot with selected product
    ...
    if st.button("Submit Composite Lot", type="primary"):
        if len(edited_df) == 0:
            ...
            try:
                ...
                # Calculate expiry date based on shortest product expiry duration
                from dateutil.relativedelta import relativedelta
                min_expiry_months = 36  # default
                for product_id in product_ids:
                    product = db.query(Product).filter(Product.id == product_id).first()
                    if product and product.expiry_duration_months:
                        min_expiry_months = min(min_expiry_months, product.expiry_duration_months)
                
                min_mfg_date = edited_df['Mfg Date'].min()
                exp_date = min_mfg_date + relativedelta(months=min_expiry_months)
